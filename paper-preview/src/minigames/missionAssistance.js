...
export function loadMissionAssistance(storage) {
    try {
        const target = storage === undefined ? globalThis.localStorage : storage;
        return createMissionAssistance(JSON.parse(target?.getItem(STORAGE_KEY) ?? '{}'));
    } catch {
        return createMissionAssistance();
    }
}
export function saveMissionAssistance(state, storage) {
    try {
        const target = storage === undefined ? globalThis.localStorage : storage;
        target?.setItem(STORAGE_KEY, JSON.stringify(createMissionAssistance(state)));
        return true;
    } catch {
        return false;
    }
